...
class RegionNode extends Node
{
    ...
    public function classifyToken($token)
    {
        ...
        } else if (($token instanceof ConstraintNode) && ($token->class == 'function')) {
            $token->phase = 'relay';
            $this->tokensByType['function'][] = $token;
        } else if (($token instanceof ConstraintNode) && ($token->class == 'logic')) {
            $token->phase = 'relay';
        ...
        } else if ($token instanceof PoolNode) {
            $this->tokensByType['pool'][] = $token;
            $token->phase = 'pool';
        } else if ($token instanceof FeatureNode) {
            $token->phase = 'feature';
