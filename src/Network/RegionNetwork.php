...
use Mesh\Element\Node\RegionNode;
use Mesh\Element\Node\FeatureNode;
use Mesh\Element\Node\RelayNode;
...
class RegionNetwork extends LinkNetwork
{
    ...
    public function getRegion($region, $baseRegion = '')
    {
        ...
        if (is_null($nodeRegion)) {
            $nodeRegion = new RegionNode($this, $baseRegion);
    ...
    public function getNextTokens($tokens)
    {
        $predictiveTokens = [];
        foreach ($tokens as $token) {
            //if (($token->wordIndex == $this->currentWord) && ($token instanceof FeatureNode) && ($token->logic != 'N')) { // não é um projection
            if (($token instanceof FeatureNode) && ($token->logic != 'N')) { // não é um projection
